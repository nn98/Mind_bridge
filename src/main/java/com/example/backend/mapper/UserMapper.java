...
public class UserMapper {
    ...
    public void applyUpdate(UserEntity user, UpdateRequest req) {
        if (req.getNickname() != null) {
            user.setNickname(norm(req.getNickname()));
        }
        if (req.getMentalState() != null) {
            user.setMentalState(emptyToNull(req.getMentalState()));
        }
        if (req.getChatGoal() != null) {
            user.setChatGoal(emptyToNull(req.getChatGoal()));
        }
        if (req.getPhoneNumber() != null) {
            user.setPhoneNumber(emptyToNull(req.getPhoneNumber()));
        }
        if (req.getFullName() != null) {
            user.setFullName(emptyToNull(req.getFullName()));
        }
        if (req.getAge() != null) {
            user.setAge(req.getAge());
        }
        if (req.getGender() != null) {
            user.setGender(emptyToNull(req.getGender()));
        }
    }
}
