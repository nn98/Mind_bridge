...
export function useChatFlow({ customUser }) {
  ...
  const [step, setStep] = useState(customUser?.email ? guestQuestions.length : 0); // 게스트만 질문 시작
  const [guestForm, setGuestForm] = useState({}); // 게스트 정보 저장
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!customUser?.email && step < guestQuestions.length) {
      const keys = ["이름", "성별", "나이", "상태", "상담받고싶은내용", "이전상담경험"];
      ...
      setChatHistory((prev) => [
        ...prev,
        {
          ...
          message: `감사합니다, ${
            guestForm["이름"] || input
          }님. 이제 상담을 시작해볼까요? 어떤 것이 가장 고민되시나요?`,
        },
      ]);
      return;
    }
