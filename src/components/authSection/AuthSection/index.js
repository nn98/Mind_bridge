...
const AuthSection = ({ type, setIsCustomLoggedIn, setCustomUser }) => {
  ...
  // 소프트 검증(비어있다고 에러 만들지 않음)
  useEffect(() => {
    validateAll(formData, { strict: false });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    formData.email, formData.password, formData.passwordConfirm,
    formData.nickname, formData.fullName, formData.age, formData.phoneNumber, formData.gender
  ]);
              ...
              <Box className="form-left">
                ...
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1 }}>
                  <TextField
                    ...
                    helperText={(submitted || touched.nickname) ? (errors.nickname || nickCheck.message || "") : ""}
                    sx={{ "& .MuiFormHelperText-root": { color: nickCheck.isAvailable && !errors.nickname ? "green" : undefined } }}
                  />
                  <Button
                ...
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1 }}>
                  <TextField
                    ...
                    sx={{ "& .MuiFormHelperText-root": { color: emailCheck.isAvailable && !errors.email ? "green" : undefined } }}
                  />
                  <Button
                    className="auth-button"
                    variant="outlined"
