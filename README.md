...
| `08/01` | 코드 확인 결과 유저의 정보를 clerk 토큰으로만 체크를 하여 null 값을 읽어서 결과 에러가 발생하였습니다
현재 로그인을 커스텀과 clerk 2가지로 하는데 조건을 clerk 으로만 잡아놓아 에러가 발생하였던 것으로 확인이 되었고 이에 조건을 설정할때 경우의 수를 잘 생각해두어야 한다 생각하였습니다. | `KJY` |
