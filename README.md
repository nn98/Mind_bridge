...
# 3. `@AuthenticationPrincipal` 어노테이션
- ### _개요_
...
| `날짜` |                                     `2025-09-09`                                      |
| `종류` |                                    `인증` `토큰` `헤더`                                     |
| `상황` |                         401 에러가 나야 할 비로그인 상태의 요청에 500 에러가 리턴됨                         |
| `원인` |            @AuthenticationPrincipal(expression = "username") String email             |
| `설명` |  해당 요청의 Auth.printipal이 User가 아닐 때<br/>(여기선 아예 토큰이 없으니 null), SpEL이 username을 찾다 터짐   |
| `해결` | 커스텀 UserDetails 도입 + expression 유지도 가능하나<br/>사실상 비로그인 요청이 일어날 일은 많지 않으니 그 요청만 예외처리 추가 |

***

# 4. `HandlerMapping` 타입 주입 후보
- ### _개요_

| `구분` |                                                            `내용`                                                            |
|:----:|:--------------------------------------------------------------------------------------------------------------------------:|
| `날짜` |                                                        `2025-09-16`                                                        |
| `종류` |                                               `Spring MVC` `Type Injection`                                                |
