...
| `날짜` |                                `2025-08-20`                                 |
| `종류` |                                `수정/보완` `프론트`                                |
| `내용` | axios / fetch 혼용중<br/>별도 사유 없을 경우 axios로 통일<br/>JWT 관련withCredentials 옵션 적용 |

| `구분` |                                                                                                                                                                                   `내용`                                                                                                                                                                                    |
|:----:|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| `날짜` |                                                                                                                                                                               `2025-08-21`                                                                                                                                                                                |
| `종류` |                                                                                                                                                                                  `진행상황`                                                                                                                                                                                   |
| `내용` | 모델 및 데이터<br/>모델 추론율 부진 - 서비스 핵심 목표 명확화 및<br/>그에 따른 데이터 적합성 평가 기준 확립 필요<br/><br/>AI Hub 데이터셋 기반 상담내용 분석 - 요인 분리 가능<br/><br/>상담 담당 모델 - 수집 데이터 기반 분석 요인 수집용 프롬프트<br/>분석 평가 처리 모델 - 상담 내용 기반 데이터 수집, 처리<br/>모델 분리 및 특화<br/><br/>번역은 근거차료 참고문헌 외부 소스 등<br/>비한글 원어 번역제공기능에 API 모델 사용<br/><br/>레이아웃 개편<br/>컴포넌트 참조방식 변경에 따른 데이터 조회/주입 누락<br/>실행흐름 분석 및 수정<br/><br/>프로젝트 구조 리팩토링 |